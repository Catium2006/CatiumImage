...
//限制上传
if ($_COOKIE["upload"] != "wait") {
    setcookie("upload", "wait", time() + 10);
    // 最大尺寸,单位是字节
    $max_size = 16777216;
    // 允许上传的图片后缀
    $allowedExts = array("gif", "jpeg", "jpg", "png", "bmp", "webp", "svg");
    $file = $_FILES["file"];
    $extension = end(explode(".", $file["name"]));     // 文件后缀名
    ...
    if (($type == "image/gif")
        ...
        if ($file["error"] > 0) {
            echo '{"status":"failed","result":"' . $file["error"] . '"}';
        } else {
            //对文件缩短文件名,取md5值的前6字符
            ...
            if (file_exists("upload/" . $file_name_short)) {
                ...
                exec_sql($sql);
                echo '{"status":"ok","result":"' . $url . '"}';
            }
        }
    } else {
        echo '{"status":"format","result":"不支持的文件格式"}';
    }
} else {
    echo '{"status":"limit","result":"上传频率过高"}';
}
